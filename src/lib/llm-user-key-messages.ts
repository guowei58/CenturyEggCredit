 ...
 */
export const USER_LLM_API_KEYS_POLICY = `I am trying to keep this app free (or as free as possible), so in-app AI features use your own API keys (saved under your login). To generate work products from your research (memos, decks, and similar outputs), you need an API key. The quality of results can vary a lot depending on which foundation model you use. You need at least one model API key.

A Google Gemini API key is free to create, but Google limits usage severely (rate limits and quotas). Claude, ChatGPT, and DeepSeek APIs are pay-as-you-go: you create a key and add funds (use provider billing) for what you use. DeepSeek is the cheapest with each API call at around half a penny, but its quality is dog doo doo and data is stale (and you're probably sending your credit analysis to the CCP). Claude Haiku 4.5 is my personal favorite but more expensive at 20-40x the cost of DeepSeek. Claude Opus 4.6 is over 100x the cost of DeepSeek.
