...
/**
 * Store a client-generated SEC filing HTML financials workbook under Saved Documents for this ticker.
 * Filename is stable per ticker + filing (form, date, accession) so repeated saves replace the same file.
 */
export async function saveSecFilingFinancialsExcelToSavedDocuments(
  userId: string,
  ticker: string,
  filing: { form: string; filingDate: string; accessionNumber: string },
  xlsxBuffer: Buffer
  ...
  const form = toSafeFilename((filing.form || "FILING").trim());
  const fdate = toSafeFilename((filing.filingDate || "nodate").trim());
  const slug = toSafeFilename(`${safeTicker}_SEC-filing-financials_html_${form}_${fdate}_${acc}`);
  const filename = `${slug}.xlsx`;
  const titleRaw = `${safeTicker} SEC filing financials (HTML) · ${filing.form} ${filing.filingDate}`;
  const title = toSafeFilename(titleRaw).slice(0, 140) || slug;
  const originalUrl = `app:sec-filing-financials/${encodeURIComponent(safeTicker)}/${encodeURIComponent(filing.accessionNumber)}`;
