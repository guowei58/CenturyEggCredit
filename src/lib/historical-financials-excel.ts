...
export type HistoricalFinancialsExcelItem = {
  id: string;
  ticker: string;
  filename: string;
  originalName: string;
...
const SUBFOLDER_NAME = "Historical Financials Excel";
const MANIFEST_FILE = "historical-financials-excel.json";
...
function toSafeFilename(raw: string): string {
  ...
  const safeBase = base.length > 0 ? base.slice(0, 120) : "historical-financials-model";
  return `${safeBase}${extOk}`;
}

async function loadManifest(userId: string, safeTicker: string): Promise<HistoricalFinancialsExcelItem[]> {
  const raw = await workspaceReadUtf8(userId, safeTicker, manifestPath());
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw) as HistoricalFinancialsExcelItem[];
...
async function writeManifest(userId: string, safeTicker: string, items: HistoricalFinancialsExcelItem[]): Promise<void> {
  await workspaceWriteUtf8(userId, safeTicker, manifestPath(), JSON.stringify(items, null, 2));
}

export async function listHistoricalFinancialsExcels(
  userId: string,
  ticker: string
): Promise<HistoricalFinancialsExcelItem[] | null> {
...
export async function getHistoricalFinancialsExcelBuffer(
  userId: string,
  ticker: string,
  filename: string
): Promise<Buffer | null> {
...
export async function saveHistoricalFinancialsExcelFile(params: {
  userId: string;
  ticker: string;
  fileBuffer: Buffer;
  originalName: string;
  maxBytes?: number;
}): Promise<{ ok: true; item: HistoricalFinancialsExcelItem } | { ok: false; error: string }> {
  const { userId, ticker, fileBuffer, originalName, maxBytes = 16_000_000 } = params;
  const safeTicker = sanitizeTicker(ticker);
  ...
  const safeOriginal = toSafeFilename(originalName || "historical-financials-model.xlsx");
  const filename = `${stamp} - ${safeOriginal}`;

  const w = await workspaceWriteFile(userId, safeTicker, blobPath(filename), fileBuffer);
  if (!w.ok) return w;

  const item: HistoricalFinancialsExcelItem = {
