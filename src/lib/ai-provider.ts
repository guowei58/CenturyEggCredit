/**
 * Shared AI backend selection (Anthropic Claude, OpenAI ChatGPT API, Google Gemini, local Ollama).
 */

export type AiProvider = "claude" | "openai" | "gemini" | "ollama";

/** Selected provider chip (Covenants, AI Chat): Claude green, ChatGPT red, Ollama blue, Gemini yellow. */
export const AI_PROVIDER_CHIP_SELECTED: Record<AiProvider, { background: string; color: string }> = {
  ...
  ollama: { background: "#2563eb", color: "#ffffff" },
};
...
export function normalizeAiProvider(raw: unknown): AiProvider | null {
  if (raw === "claude" || raw === "openai" || raw === "gemini" || raw === "ollama") return raw;
  return null;
}
