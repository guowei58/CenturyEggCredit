/**
 * Shared ixbrl-mdna-tables bundle builder (press release / mgmt deck / MD&A tables).
 * Used by the API route and Period Financials batch save — avoids HTTP loopback during batch jobs.
 */
...
export type BuildIxbrlMdnaTablesBundleOpts = {
  accessionNumber?: string;
  includeUncertainBoundaries?: boolean;
  includeLowConfidenceTables?: boolean;
};

export type IxbrlMdnaTablesBundle =
  | {
      ok: true;
      ticker: string;
      cik: string;
...
async function classifyRankedExhibit99HtmlForEarningsDisplay(
  ...
  for (let i = 0; i < orderedFilenames.length; i++) {
    ...
    if (reuse?.html && reuse.primaryDocument.trim().toLowerCase() === fn.trim().toLowerCase()) {
      html = reuse.html;
    } else {
...
export async function buildIxbrlMdnaTablesBundle(
  ticker: string,
  opts: BuildIxbrlMdnaTablesBundleOpts = {}
): Promise<IxbrlMdnaTablesBundle> {
  ...
  const acc = (opts.accessionNumber ?? "").trim();
  const includeUncertainBoundaries = opts.includeUncertainBoundaries;
  const includeLowConfidenceTables = opts.includeLowConfidenceTables;

  const filingsRes = await getAllFilingsByTicker(sym);
  if (!filingsRes) {
    return {
      ok: false,
      error: "SEC submissions not found for ticker",
      ticker: sym,
      mdnaHeadingFound: false,
      segmentHeadingFound: false,
      mdnaTableHit: false,
  ...
  if (!chosen) {
    return {
      ok: false,
      error: "No 10-K/10-Q filings found",
      ticker: sym,
      cik: filingsRes.cik,
  ...
  if (!primaryDocument) {
    return {
      ok: false,
      error: "Filing has no primary document path",
      ticker: sym,
      cik: filingsRes.cik,
      companyName: filingsRes.companyName,
      selected: {
