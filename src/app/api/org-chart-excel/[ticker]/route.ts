...
export async function GET(request: Request, { params }: { params: Promise<{ ticker: string }> }) {
  ...
  const items = await listOrgChartExcels(userId, ticker);
...
export async function POST(request: Request, { params }: { params: Promise<{ ticker: string }> }) {
  ...
  const originalName = (formData.get("filename") as string | null) ?? "org-chart-input.xlsx";
  const arrayBuffer = await blob.arrayBuffer();
  const buf = Buffer.from(arrayBuffer);

  const result = await saveOrgChartExcelFile({
    userId,
    ticker,
    fileBuffer: buf,
    originalName,
