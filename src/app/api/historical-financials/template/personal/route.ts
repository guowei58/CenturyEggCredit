...
export async function GET(request: Request) {
  try {
    ...
    console.error("[historical-financials/template/personal GET]", e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to load personal templates" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error("[historical-financials/template/personal POST]", e);
    return NextResponse.json(
