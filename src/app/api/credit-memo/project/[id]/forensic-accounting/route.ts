...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const memoTitle = `${project.ticker} — Forensic accounting review`;
  ...
  try {
    if (done.markdown) {
      await writeSavedContent(project.ticker, "forensic-accounting-latest", done.markdown, userId);
      await writeSavedContent(project.ticker, "forensic-accounting-latest-source-pack", result.sourcePack, userId);
