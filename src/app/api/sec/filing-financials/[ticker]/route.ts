...
import { fetchHtmlFilingStatements } from "@/lib/sec-filing-financials";
...
export async function GET(req: Request, { params }: { params: Promise<{ ticker: string }> }) {
  ...
  const filings = filingsRes.filings
    ...
    .filter((f) => {
      const y = Number.parseInt((f.filingDate ?? "").slice(0, 4), 10);
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : "Failed to load filing statements";
    return NextResponse.json(
      {
        ok: false,
        error: msg,
