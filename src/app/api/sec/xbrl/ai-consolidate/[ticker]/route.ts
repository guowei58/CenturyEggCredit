...
Hard rule: do not calculate or re-sum amounts except where the system message explicitly allows standalone quarters from YTD (including 4Q = FY − 9M **only for summable income-statement flows, never for weighted-average share counts** — for those use reported quarters or repeat FY weighted average in 4Q when 4Q is missing) and standalone quarterly cash flow from YTD when needed. Otherwise transcribe numbers from the files (after Latest Filing Wins). Include every Income Statement line from the CSVs (every Concept), including equity-method / affiliate and other non-operating lines — do not drop rows.
