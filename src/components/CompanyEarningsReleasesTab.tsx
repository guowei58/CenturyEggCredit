...
export function CompanyEarningsReleasesTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "earnings-releases");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "earnings-releases", trimmed);
