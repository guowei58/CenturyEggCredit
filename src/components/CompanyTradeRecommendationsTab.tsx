...
export function CompanyTradeRecommendationsTab({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "trade-recommendation");
      if (!cancelled) {
  ...
  async function handleSave() {
    ...
    await saveToServer(safeTicker, "trade-recommendation", trimmed);
  ...
  if (!safeTicker) {
    return (
      <Card title="Trade Recommendations">
        <p className="text-sm py-4" style={{ color: "var(--muted2)" }}>
          Select a company to add a trade recommendation.
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card title={`Trade Recommendations — ${safeTicker}`}>
      <SavedResponseExpandableShell title="Saved recommendation" className="min-w-0 flex-1">
        {isEditing ? (
          <>
            <RichPasteTextarea
              value={editDraft}
              onChange={setEditDraft}
              placeholder="Write or paste your trade recommendation here, then click Save."
          ...
          <>
            ...
            >
              {savedContent ? (
                ...
                <span style={{ color: "var(--muted)" }}>No saved recommendation yet.</span>
              )}
            </div>
            <button
              type="button"
