...
export function CompanyRiskFrom10kTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "risk-from-10k");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "risk-from-10k", trimmed);
  ...
  return (
    <Card title={`Risk from 10-K - ${safeTicker}`}>
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "risk-from-10k", trimmed);
