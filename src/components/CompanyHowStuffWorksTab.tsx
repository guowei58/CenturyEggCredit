...
export function CompanyHowStuffWorksTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const { template: howTemplate } = usePromptTemplateOverride("how-stuff-works", HOW_STUFF_WORKS_PROMPT_TEMPLATE);
  const prompt = safeTicker ? fillHowStuffWorksPromptPlaceholders(howTemplate, displayName, safeTicker) : "";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "how-stuff-works");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "how-stuff-works", trimmed);
  ...
  return (
    <Card title={`HowStuffWorks �?${safeTicker}`}>
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex w-full flex-shrink-0 flex-col lg:w-80">
          ...
          <PromptTemplateBox
            tabId="how-stuff-works"
            defaultTemplate={HOW_STUFF_WORKS_PROMPT_TEMPLATE}
            resolve={(tpl) =>
              safeTicker ? fillHowStuffWorksPromptPlaceholders(tpl, displayName, safeTicker) : ""
            }
            className="mb-3"
          />
          ...
          <TabPromptApiButtons
            ...
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "how-stuff-works", trimmed);
