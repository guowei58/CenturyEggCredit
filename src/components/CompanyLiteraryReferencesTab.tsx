...
/**
 * Same resolve → ingest pipeline as Recommendation / AI Memo. Saved output is shown when present;
 * regeneration runs only when the user clicks Generate / Refresh (no auto-run on tab open).
 */
export function CompanyLiteraryReferencesTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { markdown: m } = await fetchLatestGeneratedTabOutput(tk, "literaryReferences");
      setMarkdown(m);
      ...
      try {
        const res = await fetch("/api/credit-memo/project", {
          ...
          body: JSON.stringify({
            ...
            resolutionMeta,
            workProductIngestScope: "literary",
          }),
        });
  ...
  const runGenerate = useCallback(async () => {
    ...
    try {
      const url = project?.id
        ? `/api/credit-memo/project/${encodeURIComponent(project.id)}/literary-references`
        : `/api/credit-memo/ticker/${encodeURIComponent(tk)}/literary-references`;
  ...
  if (!tk) {
    return (
      <Card title="Literary References">
  ...
  return (
    <Card title={`Literary References — ${tk}`}>
      {needsSignIn ? (
      ...
      {busy ? (
        <p className="text-sm mb-3 flex items-center gap-2" style={{ color: "var(--muted)" }}>
          ...
          {genLoading
            ? "Generating literary references (best available model)…"
