...
export function CompanyLmeAnalysisTab({ ticker }: { ticker: string }) {
  ...
  /** Exact system + user strings from the last successful Run LME analysis (POST). */
  const [lastSentMessages, setLastSentMessages] = useState<{ system: string; user: string } | null>(null);
  type LmePackingStatsRow = {
  ...
  const [lastRunDiagnostics, setLastRunDiagnostics] = useState<{
    packing: LmePackingStatsRow;
    userBreakdown: LmeUserMsgBreakdown;
    systemChars: number;
  } | null>(null);
  ...
  async function regenerate() {
    ...
    try {
      ...
      const body = (await res.json()) as {
        ...
        packingStats?: LmePackingStatsRow | null;
        userMessageBreakdown?: LmeUserMsgBreakdown;
      };
    ...
    <div className="space-y-6">
      <Card title={`LME Analysis — ${safeTicker}`}>
        ...
        >
          ...
          <li>
            Click <strong>Run LME analysis</strong> to send request to AI model via API.
          </li>
        </ol>
          ...
          >
            ...
            <div className="border-t" style={{ borderColor: "var(--border2)" }}>
              <p className="px-3 py-2 mb-0 leading-relaxed" style={{ color: "var(--muted2)" }}>
                From your last <strong>Run LME analysis</strong>. The <strong>user message</strong> is two parts: (1) the
                built-in LME task spec (not taken from your tabs — it tells the model how to write the memo), then (2){" "}
                <span className="font-mono">SOURCE DOCUMENTS</span>, which is your packed workspace for that run. Refresh
                clears this panel.
              </p>
              {lastRunDiagnostics ? (
                <>
