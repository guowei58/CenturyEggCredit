...
export function CompanySuppliersTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const { template: suppliersTemplate } = usePromptTemplateOverride("suppliers", SUPPLIERS_PROMPT_TEMPLATE);
  const prompt = safeTicker
    ? suppliersTemplate.replace(/\[INSERT TICKER\]/g, safeTicker).replace(/\[INSERT COMPANY NAME\]/g, displayName)
    : "";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "suppliers");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "suppliers", trimmed);
  ...
  function openInMetaAI() {
    ...
    setStatusMessage(null);
    setClipboardFailed(false);
    void openMetaAiWithClipboard(prompt, (m) => setStatusMessage(m), () => setClipboardFailed(true));
  }
  ...
  return (
    <Card title={`Suppliers — ${safeTicker}`}>
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex w-full flex-col lg:w-80 flex-shrink-0">
          ...
          <PromptTemplateBox
            tabId="suppliers"
            defaultTemplate={SUPPLIERS_PROMPT_TEMPLATE}
            resolve={(tpl) =>
