...
export function CompanyForensicAccountingTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { markdown: m, jobId: j } = await fetchLatestGeneratedTabOutput(tk, "forensic");
      setMarkdown(m);
      setJobId(j);
  ...
  const runGenerate = useCallback(async () => {
    ...
    setGenError(null);
    try {
      const res = await fetch(`/api/credit-memo/project/${encodeURIComponent(project.id)}/forensic-accounting`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          provider,
  ...
  if (!tk) {
    return (
      <Card title="Forensic Accounting">
        <p className="text-sm py-4" style={{ color: "var(--muted2)" }}>
          Select a company with a ticker to generate a forensic accounting review from ingested filings and documents.
        </p>
      </Card>
    );
  }
  ...
  return (
    <WorkProductIngestTabLayout
      tabTitle="Forensic Accounting"
      ticker={tk}
      description={
        <>
          Same pipeline as <strong>Recommendation</strong> and <strong>AI Memo and Deck</strong>: resolve → ingest → indexed source pack.
          SEC filings, saved tab text, workspace files, and Saved Documents are included when you are signed in. The model must cite the
          pack. Latest output is stored as <code className="text-[10px]">forensic-accounting-latest.md</code> on the server.
        </>
      }
      ...
      runBusy={genLoading}
      runLabel="Generate forensic accounting review"
      runLoadingLabel="Generating forensic review…"
      resolveFailed={resolveFailed}
      ingestError={ingestError}
      genError={genError}
      jobId={jobId}
      project={project}
      outputCardTitle="Forensic accounting output"
      markdown={markdown}
      emptyOutputMessage={
        <>No saved forensic review yet for this ticker. After you generate, it stays here when you switch tabs or reload.</>
      }
    />
  );
}
