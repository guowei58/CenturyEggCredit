...
export function CompanyForensicAccountingTab({
  ...
  useEffect(() => {
    ...
    void (async () => {
      const saved = await fetchSavedFromServer(tk, "forensic-accounting-latest");
      if (saved?.trim()) setMarkdown(saved);
      const metaRaw = await fetchSavedFromServer(tk, "forensic-accounting-latest-meta");
      if (metaRaw?.trim()) {
        try {
  ...
  return (
    <div className="space-y-4">
      <Card title={`Forensic Accounting — ${tk}`}>
        <p className="mb-3 text-xs leading-relaxed" style={{ color: "var(--muted2)" }}>
          ...
          pack. Latest output is stored as forensic-accounting-latest.md on the server.
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-stretch gap-3">
          ...
          >
            {genLoading ? "Generating forensic review (may take several minutes)…" : "Generate forensic accounting review"}
          </button>
        </div>
