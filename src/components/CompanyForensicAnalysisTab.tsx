...
type SourceRow = {
  label: string;
  key?: string;
  /** Raw text length before per-source truncation. */
  charsInitial: number;
  truncated: boolean;
  isBinaryPlaceholder: boolean;
};

type ForensicGetResponse = {
  ticker: string;
  sourceInventory: SourceRow[];
  /** Sum of per-block saved/extracted lengths (`charsInitial`), not post-truncation totals. */
  totalChars: number;
...
export function CompanyForensicAnalysisTab({ ticker, companyName }: { ticker: string; companyName?: string }) {
  const safeTicker = ticker?.trim() ?? "";
  const { status: authStatus } = useSession();
  const { ready: prefsReady, preferences, updatePreferences } = useUserPreferences();
  const [data, setData] = useState<ForensicGetResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [generating, setGenerating] = useState(false);
  const [error, setError] = useState<string | null>(null);
  /** Exact system + user strings from the last successful forensic run (POST). */
  const [lastSentMessages, setLastSentMessages] = useState<{ system: string; user: string } | null>(null);
  type ForensicPackingStatsRow = {
  ...
  const [lastRunDiagnostics, setLastRunDiagnostics] = useState<{
    packing: ForensicPackingStatsRow;
    userBreakdown: ForensicUserMsgBreakdown;
    systemChars: number;
  } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/forensic-analysis/${encodeURIComponent(safeTicker)}`);
      const body = (await res.json()) as ForensicGetResponse & { error?: string };
      if (!res.ok) throw new Error(body.error ?? "Failed to load forensic analysis");
      setData(body);
    } catch (e) {
  ...
  async function regenerate() {
    ...
    try {
      ...
      const body = (await res.json()) as {
        ...
        packingStats?: ForensicPackingStatsRow | null;
        userMessageBreakdown?: ForensicUserMsgBreakdown;
      };
  ...
  return (
    <div className="space-y-6">
      <Card title={`Forensic Analysis — ${safeTicker.trim().toUpperCase()}`}>
        ...
        >
          ...
          <li>
            Click <strong>Run forensic analysis</strong> to send request to AI model via API.
          </li>
        </ol>

        {needsSignIn && (
          <p className="text-xs mb-4 rounded border px-3 py-2" style={{ borderColor: "var(--warn)", color: "var(--muted2)" }}>
            Sign in to load saved sources and run analysis. Your saved tab content is stored per account.
          </p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          >
            {generating ? "Running forensic analysis…" : "Run forensic analysis"}
          </button>
          <button
            type="button"
            onClick={() => void load()}
        ...
        <div className="rounded border mb-4 text-xs" style={{ borderColor: "var(--border2)" }}>
          <div
            ...
            title="Total is the sum of each row’s Chars column (full saved or extracted length on refresh; Generate applies model caps and retrieval)."
          >
            {data ? (
              <>
          ...
          {!prefsReady ? (
            ...
            <>
              ...
              {!data.hasSubstantiveText && !needsSignIn && (
                <p className="px-3 py-2 text-[11px]" style={{ color: "var(--muted)" }}>
                  No substantive text yet. Upload non-Excel files to your workspace and/or save tab content (generated tab
                  files are excluded), then refresh.
                </p>
              )}
            </>
          ) : (
          ...
          >
            ...
            <div className="border-t" style={{ borderColor: "var(--border2)" }}>
              <p className="px-3 py-2 mb-0 leading-relaxed" style={{ color: "var(--muted2)" }}>
                From your last <strong>Run forensic analysis</strong>. The <strong>user message</strong> is two
                parts: (1) the built-in forensic task spec (ticker, company name if provided, and output rules), then (2){" "}
                <span className="font-mono">EVIDENCE (SOURCE PACK)</span> plus file inventory, from your workspace/tabs
                corpus for that run. Refresh clears this panel.
              </p>
              {lastRunDiagnostics ? (
                <>
                  <div
              ...
              <p className="px-3 py-2 text-[10px] leading-relaxed border-t" style={{ borderColor: "var(--border2)", color: "var(--muted)" }}>
                <strong>{lastRunDiagnostics.packing.bundleCharCap.toLocaleString()}</strong> is a <em>ceiling</em> on the
                sum of packed block bodies. Your run totalled{" "}
                <strong>{lastRunDiagnostics.packing.packedPartsCharSum.toLocaleString()}</strong> because that is how
                much text remained after per-source limits, retrieval (where applicable), and bundle trimming—not
                because the system “shrinks” to 400k by default. The user message also adds the task spec, file inventory, and{" "}
      ...
      {data?.cachedMarkdown?.trim() ? (
        <Card title="Forensic analysis output">
          <div className="prose-covenants text-sm leading-relaxed max-w-none" style={{ color: "var(--text)" }}>
            <SavedRichText content={data.cachedMarkdown} ticker={safeTicker.trim().toUpperCase()} />
          </div>
        </Card>
      ) : (
        <Card title="Forensic analysis output">
