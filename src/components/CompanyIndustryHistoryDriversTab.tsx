...
export function CompanyIndustryHistoryDriversTab({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "industry-history-drivers");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "industry-history-drivers", trimmed);
        ...
        <div className="flex w-full flex-col lg:w-80 flex-shrink-0">
          ...
          <TabPromptApiButtons
            ...
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "industry-history-drivers", trimmed);
