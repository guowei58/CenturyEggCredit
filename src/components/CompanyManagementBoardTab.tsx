...
export function CompanyManagementBoardTab({ ticker }: { ticker: string }) {
  ...
  const { template: mgmtBoardTemplate } = usePromptTemplateOverride("management-board", MANAGEMENT_BOARD_PROMPT_TEMPLATE);
  const prompt = safeTicker ? fillCompanyPromptTemplate(mgmtBoardTemplate, safeTicker) : "";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "management-board");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "management-board", trimmed);
    ...
    <Card title={`Management & Board - ${safeTicker}`}>
      <TabPromptSlideOutShell
        ...
        prompt={
          ...
          <PromptTemplateBox
            tabId="management-board"
            defaultTemplate={MANAGEMENT_BOARD_PROMPT_TEMPLATE}
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            />
            <TabPromptApiButtons
            researchSaveKey="management-board"
            userPrompt={prompt}
            onResult={() => {
              setClipboardFailed(false);
            }}
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "management-board", trimmed);
