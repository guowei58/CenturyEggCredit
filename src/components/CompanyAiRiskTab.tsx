...
const TAB_ID = "ai-risk";

export function CompanyAiRiskTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const { template: aiRiskTemplate } = usePromptTemplateOverride(TAB_ID, AI_RISK_PROMPT_TEMPLATE);
  const prompt = safeTicker ? fillCompanyPromptTemplate(aiRiskTemplate, safeTicker, companyName) : "";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, TAB_ID);
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, TAB_ID, trimmed);
      ...
      <TabPromptSlideOutShell
        ...
        prompt={
          <>
            ...
            <PromptTemplateBox
              tabId={TAB_ID}
              defaultTemplate={AI_RISK_PROMPT_TEMPLATE}
            ...
            <TabPromptApiButtons
              ...
              persistAfterResult={async (text) => {
                ...
                if (!safeTicker) return;
                const ok = await saveToServer(safeTicker, TAB_ID, trimmed);
