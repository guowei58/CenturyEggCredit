...
/** Same resolve → ingest pipeline as How to Look Like a Dumbass; earnings transcript prompt server-side. */
export function CompanyNextQuarterEarningsTranscriptTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { markdown: m } = await fetchLatestGeneratedTabOutput(tk, "nextQuarterEarningsTranscript");
      setMarkdown(m);
      ...
      try {
        const res = await fetch("/api/credit-memo/project", {
          ...
          body: JSON.stringify({
            ...
            resolutionMeta,
            workProductIngestScope: "earnings-transcript",
          }),
        });
  ...
  const runGenerate = useCallback(async () => {
    ...
    try {
      const url = project?.id
        ? `/api/credit-memo/project/${encodeURIComponent(project.id)}/next-quarter-earnings-transcript`
        : `/api/credit-memo/ticker/${encodeURIComponent(tk)}/next-quarter-earnings-transcript`;
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          companyName: companyName?.trim() ?? "",
          nextQuarter: nextQuarter.trim(),
        }),
      });
      const data = (await res.json()) as {
        ok?: boolean;
  ...
  }, [project, companyName, nextQuarter, tk, updatePreferences]);

  if (!tk) {
    return (
      <Card title="Next Quarter Earnings Transcript">
  ...
  return (
    <Card title={`Next Quarter Earnings Transcript — ${tk}`}>
      {needsSignIn ? (
        <p className="text-xs mb-3 rounded border px-3 py-2" style={{ borderColor: "var(--warn)", color: "var(--muted2)" }}>
          ...
          enough, saved tab <strong>.txt</strong> response files, ROIC.AI historical transcripts (when configured), and other ingested
          files are used.
        </p>
      ) : null}
      ...
      {!needsSignIn ? (
        <div className="mb-3 space-y-3">
          <label className="block text-xs" style={{ color: "var(--muted2)" }}>
            Next quarter to forecast (optional)
            <input
              type="text"
              value={nextQuarter}
              onChange={(e) => setNextQuarter(e.target.value)}
              placeholder="e.g. 2026Q2 or Q2 FY2026"
              className="mt-1 w-full max-w-md rounded border bg-[var(--card2)] px-2 py-1.5 text-sm text-[var(--text)] focus:border-[var(--accent)] focus:outline-none"
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {!project && !genLoading ? (
              <span className="text-xs" style={{ color: "var(--muted2)" }}>
                Uses your saved credit memo plus saved tab responses. Folder ingest and ROIC.AI transcripts improve context but are not
                required to run Generate.
              </span>
            ) : null}
          </div>
        </div>
      ...
      {busy ? (
        <p className="text-sm mb-3 flex items-center gap-2" style={{ color: "var(--muted)" }}>
          ...
          {genLoading
            ? "Generating next-quarter earnings transcripts (best available model)…"
      ...
      {markdown?.trim() ? (
        ...
        <p className="text-sm" style={{ color: "var(--muted2)" }}>
          No output yet. Click <strong>Generate</strong> once you have a saved memo on <strong>AI Memo and Deck</strong> and/or saved tab
          responses.
        </p>
      ) : null}
    </Card>
