...
export function CompanyStartupRisksTab({
  ticker,
}: {
  ticker: string;
}) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "startup-risks");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "startup-risks", trimmed);
  ...
  return (
    <Card title={`Startup Risks - ${safeTicker}`}>
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "startup-risks", trimmed);
