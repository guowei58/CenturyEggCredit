...
export function CompanyResearchRoadmapTab({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "research-roadmap");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "research-roadmap", trimmed);
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            />
            <TabPromptApiButtons
            researchSaveKey="research-roadmap"
            userPrompt={prompt}
            onResult={() => {
              setClipboardFailed(false);
            }}
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "research-roadmap", trimmed);
