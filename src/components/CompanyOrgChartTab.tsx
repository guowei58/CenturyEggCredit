...
import {
  OrgChartExcelProvider,
  OrgChartExcelUpload,
  OrgChartExcelViewer,
  OrgChartTabPromptApiButtons,
  useOrgChartExcel,
} from "@/components/OrgChartExcelFileBox";
...
export function CompanyOrgChartTab({
  ...
  const resolvePromptPreview = useCallback(
    (tpl: string) =>
      safeTicker ? resolveOrgChartTemplate(tpl, { ticker: safeTicker, companyName, appOrigin }) : "",
    [safeTicker, companyName, appOrigin]
  );
    ...
    return (
      <Card title="Org Chart">
        <p className="text-sm py-4" style={{ color: "var(--muted2)" }}>
          Select a company to build the org chart prompt and Excel deliverable.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <OrgChartExcelProvider ticker={safeTicker}>
        <Card
          className="card-shell--excel-workbook"
          title={`Org Chart - ${safeTicker}`}
          titleAside={
