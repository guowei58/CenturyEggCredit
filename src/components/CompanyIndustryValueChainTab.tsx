...
export function CompanyIndustryValueChainTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const { template: valueChainTemplate } = usePromptTemplateOverride(
    "industry-value-chain",
    INDUSTRY_VALUE_CHAIN_PROMPT_TEMPLATE
  );
  const prompt = safeTicker ? resolveIndustryValueChainTemplate(valueChainTemplate, safeTicker, companyName) : "";
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "industry-value-chain");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "industry-value-chain", trimmed);
  ...
  if (!safeTicker) {
    return (
      <Card title="Industry Value Chain">
  ...
  return (
    <Card title={`Industry Value Chain - ${safeTicker}`}>
      <TabPromptSlideOutShell
        ...
        prompt={
          ...
          <PromptTemplateBox
            tabId="industry-value-chain"
            defaultTemplate={INDUSTRY_VALUE_CHAIN_PROMPT_TEMPLATE}
            resolve={(tpl) => (safeTicker ? resolveIndustryValueChainTemplate(tpl, safeTicker, companyName) : "")}
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            />
            <TabPromptApiButtons
            researchSaveKey="industry-value-chain"
            userPrompt={prompt}
            onResult={() => {
              setClipboardFailed(false);
            }}
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "industry-value-chain", trimmed);
