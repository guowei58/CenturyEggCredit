...
const SAVE_KEY = "company-reputation" as const;

export function CompanyReputationTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const { template: promptTemplate } = usePromptTemplateOverride(
    SAVE_KEY,
    COMPANY_REPUTATION_PROMPT_TEMPLATE
  );
  ...
  return (
    <Card title={`Company Reputation - ${safeTicker}`}>
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex w-full flex-col lg:w-80 flex-shrink-0">
          ...
          <PromptTemplateBox
            tabId={SAVE_KEY}
            defaultTemplate={COMPANY_REPUTATION_PROMPT_TEMPLATE}
            resolve={(tpl) =>
