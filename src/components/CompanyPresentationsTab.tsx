...
function linkify(text: string): ReactNode[] {
  ...
  return parts.map((part, i) =>
    /^https?:\/\//.test(part) ? (
      <a key={i} href={part} target="_blank" rel="noopener noreferrer" className="underline break-all" style={{ color: "var(--accent)" }}>
        {part}
      </a>
    ) : (
      part
...
export function CompanyPresentationsTab({
  ticker,
  companyName,
}: {
  ticker: string;
  ...
  const prompt = useMemo(() => {
    ...
    return presentationsTemplate.replace(/\{\{TICKER\}\}/g, safeTicker).replace(
      /\{\{COMPANY_NAME\}\}/g,
      companyNameLine
    );
  }, [safeTicker, companyNameLine, presentationsTemplate]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "presentations");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "presentations", trimmed);
    ...
    <Card title={`Presentations — ${safeTicker}`}>
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex w-full flex-col lg:w-80 flex-shrink-0">
          ...
          <PromptTemplateBox
            tabId="presentations"
            defaultTemplate={MGMT_PRESENTATIONS_PROMPT_TEMPLATE}
            resolve={(tpl) =>
              safeTicker
          ...
          <div className="tab-prompt-ai-actions-grid mb-2">
            ...
            <button type="button" onClick={openInMetaAI} className="tab-prompt-ai-action-btn" style={{ borderColor: "#0866FF", color: "#0866FF", background: "transparent" }}>
              Open in Meta AI
            </button>
