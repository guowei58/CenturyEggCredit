...
export function CompanyOutOfTheBoxIdeasTab({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const loaded = await fetchSavedTabContent(safeTicker, "out-of-the-box-ideas");
      if (!cancelled) {
  ...
  async function handleSaveResponse() {
    ...
    if (!safeTicker) return;
    await saveToServer(safeTicker, "out-of-the-box-ideas", trimmed);
  ...
  if (!safeTicker) {
    return (
      <Card title="Out-of-the-Box Ideas">
  ...
  return (
    <Card title={`Out-of-the-Box Ideas - ${safeTicker}`}>
...
<TabPromptOpenInAiButtons
  ...
  setStatusMessage={setStatusMessage}
            ...
            />
            <TabPromptApiButtons
            researchSaveKey="out-of-the-box-ideas"
            userPrompt={prompt}
            onResult={() => {
              setClipboardFailed(false);
            }}
            persistAfterResult={async (text) => {
              ...
              if (!safeTicker) return;
              const ok = await saveToServer(safeTicker, "out-of-the-box-ideas", trimmed);
