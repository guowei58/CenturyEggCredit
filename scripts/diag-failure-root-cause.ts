/**
 * Deep root-cause diagnosis for a single 10-Q probe failure.
 * Usage: npx tsx scripts/diag-failure-root-cause.ts ETN 2024-03-31
 */
...
const ticker = (process.argv[2] ?? "ETN").toUpperCase();
const period = process.argv[3] ?? "2024-03-31";
...
async function main() {
  ...
  console.log("bundle:", bundle.statements.map((s) => `${s.id}@${s.sourceHtmlFile}(${s.rows.length}r)`).join(", "));
  ...
  if (located.packet) {
    ...
    for (const table of block.tables.slice(0, 3)) {
      ...
      const { parsed, validated } = __test_parsePrimaryStatementAtTableOffset(html, missingKind, ti, "10-Q");
  ...
  if (cluster) {
    ...
    const shape = validated ? __test_validateSinglePrimaryStatementShape(validated, "10-Q") : false;
    console.log(
      `  clu ${missingKind} #${ti}@${hit.table.offset} rows=${parsed?.rows.length ?? 0} shape=${shape}`
    );
    if (validated && !shape) {
      console.log("    labels:", validated.rows.slice(0, 10).map((r) => r.label).join(" | "));
    }
  ...
  // Scan all tables in section for candidate IS/BS/CF tables
  ...
  for (const c of candidates.filter((x) => !x.shape).slice(0, 3)) {
    const { validated } = __test_parsePrimaryStatementAtTableOffset(html, missingKind, c.ti, "10-Q");
  ...
  if (!got && stmtReports.length > 0) {
    ...
    for (const rep of stmtReports.slice(0, 2)) {
      ...
      for (let ti = 0; ti < repCtx.tables.length; ti++) {
        const { parsed, validated } = __test_parsePrimaryStatementAtTableOffset(repHtml, missingKind, ti, "10-Q");
        if (!parsed) continue;
        const shape = validated ? __test_validateSinglePrimaryStatementShape(validated, "10-Q") : false;
