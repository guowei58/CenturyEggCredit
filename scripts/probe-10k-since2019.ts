/**
 * Batch-probe 10-K HTML face parsing for tickers since 2019.
 * Usage: npx tsx scripts/probe-10k-since2019.ts [TICKERS] [SINCE_YEAR]
 */
...
const DEFAULT_TICKERS = ["AAPL", "BHC", "FICO", "GEN", "HTZ", "LUMN"];
...
async function probeFiling(
  ...
  try {
    ...
    const section = locateFinancialStatementsSection(ctx, "10-K");
    const located = locatePrimaryStatementPacket(ctx, { form: "10-K" });
...
async function main() {
  ...
  for (const ticker of tickers) {
    ...
    let filings = res.filings
      .filter((f) => f.form === "10-K" && onOrAfterSince(f.filingDate, f.reportDate))
    ...
    console.log(`\n--- ${ticker}: ${filings.length} 10-K filings since ${SINCE}${capNote} ---`);
  ...
  const outPath = `scripts/probe-10k-since2019-results-${Date.now()}.json`;
